...
inline bool EWatchdog::isActive() const {
  return _active;
}
 
inline void EWatchdog::setInterval(Interval interval) {
  IWDG_WriteAccessEnable();  
  *_interval_ptr = interval;
  IWDG_WriteAccessDisable();  
}

void EWatchdog::begin() {
  begin((Interval)((*_interval_ptr)&RELOAD_MASK));
}
