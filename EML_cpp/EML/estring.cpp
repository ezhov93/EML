...
EStringSumHelper & operator + (const EStringSumHelper &lhs, const __FlashEStringHelper *rhs)
{
	EStringSumHelper &a = const_cast<EStringSumHelper&>(lhs);
...
unsigned char EString::startsWith( const EString &s2, unsigned int offset ) const
{
...
char EString::charAt(unsigned int loc) const
{
	return operator[](loc);
}
...
void EString::getBytes(unsigned char *buf, unsigned int bufsize, unsigned int index) const
{
...
int EString::indexOf(char c) const
{
	return indexOf(c, 0);
}
...
int EString::indexOf(const EString &s2) const
{
	return indexOf(s2, 0);
}
