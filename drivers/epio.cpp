...
static inline void setupAdc(Pin pin) {
  if (!useAdc) {
    const int freq = 1;  // MHz
    const int presc = log2((float)(HSE2_Value / freq));
    ADC_InitTypeDef adc;
    ADCx_InitTypeDef adcX;

    RST_CLK_ADCclkSelection(RST_CLK_ADCclkCPU_C1);
    RST_CLK_ADCclkPrescaler(presc << 3);
    ...
    adcX.ADC_ClockSource = ADC_CLOCK_SOURCE_ADC;
    ...
    adcX.ADC_VRefSource =
        useAdcExt ? ADC_VREF_SOURCE_EXTERNAL : ADC_VREF_SOURCE_INTERNAL;
    adcX.ADC_IntVRefSource = ADC_INT_VREF_SOURCE_INEXACT;
    adcX.ADC_Prescaler = ADC_CLK_div_None;
...
static inline void setupDac(Pin pin) {
  if (!useDac1 && pinMap[pin].chDac == 1) {
    ...
    bitSet(MDR_DAC->CFG, 2);
    useDac1 = true;
  } else if (!useDac2 && pinMap[pin].chDac == 2) {
    RST_CLK_PCLKcmd(RST_CLK_PCLK_DAC, ENABLE);
    bitSet(MDR_DAC->CFG, 3);
    useDac2 = true;
  }
}
