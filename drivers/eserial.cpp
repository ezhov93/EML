...
void ESerial::begin() { begin(_baudRate); }
...
void ESerial::begin(uint32 baudrate) {
  ...
  uint32 div = 0;
  bool hse = RST_CLK_GetFlagStatus(RST_CLK_FLAG_HSERDY);
  if (HSE_Value > 12000000) div = UART_HCLKdiv4;

  UART_BRGInit(serial, div);
...
inline void ESerial::setBaudRate(uint32 baudrate) { _baudRate = baudrate; }

inline int ESerial::baudrate() const { return _baudRate; }

inline void ESerial::setDataBits(DataBits dataBits) { _dataBits = dataBits; }

inline void ESerial::setParity(Parity parity) { _parity = parity; }

inline void ESerial::setStopBits(StopBits stopBits) { _stopBits = stopBits; }

inline ESerial::DataBits ESerial::dataBits() const { return _dataBits; }

inline ESerial::Parity ESerial::parity() const { return _parity; }

inline ESerial::StopBits ESerial::stopBits() const { return _stopBits; }

int ESerial::read(void) {
  byte ch;
  ERingPrivate *ringRx = (ERingPrivate *)_rx.ring;
  if (ERingPrivate::pop(*ringRx, &ch, sizeof(ch)))
    return ch;
  else
    return -1;
}
...
int ESerial::availableForWrite(void) {
  ERingPrivate *ringTx = (ERingPrivate *)_tx.ring;
  return ERingPrivate::count(*ringTx);
}

size_t ESerial::write(byte ch) {
  write(&ch, sizeof(byte));
  return 1;
}

size_t ESerial::write(const void *buf, uint32 len) {
  MDR_UART_TypeDef *serial = (MDR_UART_TypeDef *)_serial_ptr;
  ERingPrivate *ringTx = (ERingPrivate *)_tx.ring;
  int cnt = availableForWrite();
  bool error = !ERingPrivate::push(*ringTx, (byte *)buf, len);
  while (cnt++ < FIFO_SIZE && !UART_GetFlagStatus(serial, UART_FLAG_TXFF)) {
    byte ch;
    bool error = !ERingPrivate::pop(*ringTx, &ch, 1);
    if (!error && ch == '\n') __nop();
    if (!error) UART_SendData(serial, ch);
  }
  return !error;
}
...
void ESerial::flush(void) {
  MDR_UART_TypeDef *serial = (MDR_UART_TypeDef *)_serial_ptr;
  ERingPrivate *ringTx = (ERingPrivate *)_tx.ring;
  while (!ERingPrivate::isEmpty(*ringTx))
    ;  // wait for TX buffer empty
  while (!UART_GetFlagStatus(serial, UART_FLAG_TXFE))
    ;  // wait for Transmite
}
...
bool Write(Uart *uart) {
  byte ch;
  bool error = !ERingPrivate::pop(*uart->ringTx_ptr, &ch, sizeof(ch));
  if (error) return false;
  if (uart->base) UART_SendData(uart->base, ch);
  return true;
}
...
static void Handler(Uart *uart) {
    ...
    while (!UART_GetFlagStatus(uart->base, UART_FLAG_TXFF)) {
      if (!Write(uart)) {
        if (uart->handlers.trm) {
          while (!UART_GetFlagStatus(uart->base, UART_FLAG_TXFE))
            uart->handlers.trm();
        }
        break;
      }
  ...
  if (UART_GetITStatus(uart->base, UART_IT_RX)) {
    while (!UART_GetFlagStatus(uart->base, UART_FLAG_RXFE)) Read(uart);
    UART_ClearITPendingBit(uart->base, UART_IT_RX);
  }
  if (UART_GetITStatus(uart->base, UART_IT_RT)) {
    while (!UART_GetFlagStatus(uart->base, UART_FLAG_RXFE)) Read(uart);
    if (uart->handlers.recv) uart->handlers.recv();
...
void UART1_IRQHandler() { Handler(&gUart[0]); }

void UART2_IRQHandler() { Handler(&gUart[1]); }
