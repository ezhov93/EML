...
ETimer::ETimer(int timerId, Type type) : _timerId(timerId), _type(type) {
  ...
  *_interval_ptr = 1000;
  ...
  _rate = 1;
  /* pow(2,div) */
  for (int rate = 0; rate < _div; ++rate) _rate *= 2;
}

inline int ETimer::interval() const { return *_interval_ptr; }
...
inline bool ETimer::isSingleShot() const { return _singleShot; }

bool ETimer::isReady() const {
  MDR_TIMER_TypeDef* timer = (MDR_TIMER_TypeDef*)_timer_ptr;
  return (TIMER_GetStatus(timer) & TIMER_STATUS_CNT_ARR) ==
...
void ETimer::start() { start(*_interval_ptr); }
...
static void Handler(MDR_TIMER_TypeDef* base, int id) {
  ...
  if (handlers_ptr[id - 1] != null) {
    handlers_ptr[id - 1]();
    if (singleShots[id - 1]) TIMER_Cmd(base, DISABLE);
  }
...
#if defined(USE_MDR1986VE1T) || defined(USE_MDR1986VE3)
void TIMER4_IRQHandler() { Handler(MDR_TIMER4, 4); }
#endif
